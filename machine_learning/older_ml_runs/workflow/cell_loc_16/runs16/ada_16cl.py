...
Run ml with pipeline and other objects to perform the entire machine 
learning workflow without data leakage. 
Column transformer, ml model (maybe)and grid search have the n_jobs parameter.
...
DATA_FOLDER = './data_16'
SCORES_FILE = 'ada_cell16_scores.txt'
...
for a_file in os.listdir(DATA_FOLDER):
    ...
    clf = Pipeline(steps=[('preprocessor', preprocessor),
                          ('ada',
                           AdaBoostClassifier(base_estimator=dt,
                                               random_state=42))
                          ])
