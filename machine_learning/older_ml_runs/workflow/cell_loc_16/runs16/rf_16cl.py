...
Run RF model with pipeline and other objects to perform the entire machine 
learning workflow without data leakage. 
Column transformer, random forest and grid search have the n_jobs parameter.

RF uses balanced class weights, 1 process. 
...
DATA_FOLDER = './data_16'
SCORES_FILE = 'rf_cell16_scores.txt'
...
for a_file in os.listdir(DATA_FOLDER):
    ...
    grid = {
        'classifier__ccp_alpha': [0, 0.0002, 0.001],
        'classifier__max_features': [40, 80, 200],
