...
Default hyperparameters, but with broad a range of ccp_alpha
RF uses 60 processes
"""
import sys
import pandas as pd
...
rf_grid = {
    'classifier__ccp_alpha': [0, 0.001, 0.003, 0.005, 0.007, 0.01, 0.02, 0.05]
}

# Pipleline to perform full workflow from preprocessing to model fitting
clf = Pipeline(steps=[('preprocessor', preprocessor),
                      ('classifier',
                       RandomForestClassifier(random_state=42, n_jobs=60))
                     ])
