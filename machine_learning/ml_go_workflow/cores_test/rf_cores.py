...
Customised for go terms, use selected hps, varies number of cores
used

Modified from rf_oob.py in
~/machine_learning/ml_go_workflow/hp_test20
"""
import sys
import numpy as np
import pandas as pd
...
# Create models and container to hold them
models = []
for n in [128, 64, 32, 16, 8, 4, 2, 1]:
    rf = RandomForestClassifier(ccp_alpha=0.001, max_features=0.2,
                                ...
                                n_jobs=n, class_weight='balanced')
    temp = (n, rf, 'rf')
    models.append(temp)
...
for model in models:
    print(model[2], model[0], 'started:', get_time())
    time_start = get_time()
    ...
    # Appending score
    y_pred = np.argmax(clf[model[2]].oob_decision_function_, axis=1)
    ...
    run = pd.Series([model[2] + str(model[0]), clf[model[2]].oob_score_,
    ...
    fi = clf[model[2]].feature_importances_
    fi_sort = pd.Series(fi, index=X.columns).sort_values(ascending=False)
    fi_sort.name = model[2] + str(model[0])
