...
Customised for non DGE and GO class labels, use selected hps and a
...
CORES = 3  # Num of cores for rf

# Modified this code, as this script is for DGE term prediction
class_label = sys.argv[1]
fi_file = OUTPUT_FOLDER + JOB_ID + '_fi.txt'
scores_file = OUTPUT_FOLDER + JOB_ID + '_scores.txt'
model_file = OUTPUT_FOLDER + JOB_ID + '_model'
...
# Reads in ml data file
data = pd.read_csv(ML_FILE, sep='\t', index_col=0)
...
# Create model
rf = RandomForestRegressor(ccp_alpha=0.001, max_features=0.2,
...
# Replace missing class targets with their median
y = y.fillna(y.median())
# Training
clf.fit(X, y)
...
run = pd.Series([model[0], clf[model[0]].oob_score_,
                time_start, time_end],
                index=['model_name', 'oob_r_sq',
                       'time_start', 'time_end'])
